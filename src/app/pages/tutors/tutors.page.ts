...
export class TutorsPage implements OnInit {
  ...
  extractSpecialties() {
    this.specialties = [...new Set(this.tutors.map((tutor) => tutor.speciality))].sort()
  }
  ...
  applyFilters() {
    this.filteredTutors = this.tutors.filter((tutor) => {
      ...
      const nameMatch = this.searchTerm
        ? (tutor.first_name + " " + tutor.last_name).toLowerCase().includes(this.searchTerm.toLowerCase())
        : true

      // Filtro por especialidad
      const specialtyMatch = this.selectedSpecialty ? tutor.speciality === this.selectedSpecialty : true
  ...
  get paginatedTutors(): Tutor[] {
    ...
    return this.filteredTutors.slice(startIndex, startIndex + this.itemsPerPage)
  }
  ...
  getStatusItems(tutor: Tutor): { icon: string; label: string; checked: boolean }[] {
    return [
